...
interface ExperienceProps {
    prevExperience: ExperienceData,
    onSave: any,
    edit: boolean,
    loading: boolean,
    closeModal: any 
}
const ExperienceFormComponent = ({ prevExperience, onSave, edit, loading, closeModal }: ExperienceProps) => {
  const [experience, setExperience] = useState<Experience | ExperienceData>( edit ? prevExperience : {
    position: '',
    company: '',
    summary: '',
    startDate: '',
    ...
    <form onSubmit={handleSubmit} className="w-full mx-auto overflow-auto">
      ...
      <div className="flex flex-col mb-2">
        ...
        <input
          ...
          onChange={(event) =>
            setExperience({ ...experience, location: event.target.value })
          }
          className="rounded-3xs bg-white font-serif font-medium box-border w-full flex flex-row  py-[0.9rem] px-[1rem] items-center justify-end border-[1px] border-solid border-gainsboro-200"
